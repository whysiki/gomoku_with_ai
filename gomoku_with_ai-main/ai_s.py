...
class AlphaBetaGomokuAI:
    ...
    def update_TTable(self, table: dict, hash: int, score: int, depth: int):
        if hash not in table or depth >= table[hash][1]:
            table[hash] = [score, depth]  # 存储最佳下法
    ...
    def alphabeta(
        ...
        if (
            ...
            print(f"[blue]Hit TTable : {self.rollingHash: {self.rollingHash}}[/blue]")
        ...
        if is_max_player:
            max_val = -np.inf
            # actions = get_near_actions_with_noempty(
            #     board_tuple, 2
            # )  # 获取当前有棋子的位置周围的位置
...
#         result = pool.apply_async(get_best_action_multiprocess, (ai, board_state))
#         best_action = result.get()
